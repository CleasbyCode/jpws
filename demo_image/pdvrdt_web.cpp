...
struct ProgramArgs {
	...
	static std::optional<ProgramArgs> parse(int argc, char** argv) {
        ...
        if (cmd == "recover") {
        	...
        	out.pin_file_path = fs::path(arg(3));
...
static std::string decryptDataFile(std::vector<uint8_t>& png, bool isMastodonFile, bool& hasDecryptionFailed) {
    ...
    uint8_t 
    	pin_file_vec_index = 0,
    	sodium_keys_length = 48;
    	
    uint64_t pin = getValue(pin_file_vec, pin_file_vec_index, BYTE_SIZE);
    
    // Get the PIN value from the PIN vector (user PIN input from Web form)
    //for (uint8_t i = 0; i < 8; ++i) {
    //	pin = (pin << 8) | static_cast<uint64_t>(pin_file_vec[pin_file_vec_index + i]);
    // }
    ...
    size_t
    	sodium_xor_key_pos = SODIUM_KEY_INDEX,
    	sodium_key_pos     = SODIUM_KEY_INDEX + BYTE_SIZE;
...
int main(int argc, char** argv) {
	try {
		...
		if (args.mode == Mode::conceal) {  
			...
			if (!fs::exists(args.pin_file_path)) {
				throw std::runtime_error("Pin File Error: File not found.");
			}

			if (!hasValidFilename(args.pin_file_path)) {
				throw std::runtime_error("Invalid Input Error: Unsupported characters in filename arguments.");
			}

			if (!hasFileExtension(args.pin_file_path, {".txt"})) {
				throw std::runtime_error("File Type Error: Invalid image extension. Only expecting \".txt\".");
			}

			std::ifstream pin_file_ifs(args.pin_file_path, std::ios::binary);

    			if (!pin_file_ifs) {
    				throw std::runtime_error("Read File Error: Unable to read PIN file. Check the filename and try again.");
   			}

			uintmax_t pin_file_size = fs::file_size(args.pin_file_path);

    			constexpr uint8_t
				MAX_PIN_FILE_SIZE = 24,
				MIN_PIN_FILE_SIZE = 8;

			if (MIN_PIN_FILE_SIZE > pin_file_size || pin_file_size > MAX_PIN_FILE_SIZE) {
				throw std::runtime_error("PIN File Error: Invalid file size.");
    			}

    			std::vector<uint8_t> pin_file_vec(pin_file_size);

			pin_file_ifs.read(reinterpret_cast<char*>(pin_file_vec.data()), pin_file_size);
			pin_file_ifs.close();

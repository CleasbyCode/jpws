...
static void updateValue(std::vector<uint8_t>& vec, uint32_t insert_index, uint64_t NEW_VALUE) {
...
static void imageColorCheck(std::vector<uint8_t>& png, bool &hasLodeModifiedImage, bool& hasTwitterBadDims) {
 	...
 	if (color_type == TRUECOLOR_RGB || color_type == TRUECOLOR_RGBA) {	
		...
		hasTwitterBadDims = (height > TWITTER_MAX_RGB_DIMS || width > TWITTER_MAX_RGB_DIMS || TWITTER_MIN_DIMS > height || TWITTER_MIN_DIMS > width);
    		...
    		if (stats.numcolors <= 256) {
    			hasLodeModifiedImage = true;
        		std::vector<uint8_t> 
        			indexed_image(width * height),
        			palette;
...
static void removeSuperfluousChunks(std::vector<uint8_t>& png) {				
	...
	// First check for and remove any trialing data after IEND.		
    	...
    	// We make a copy of the cover image, only keeping required chunks, then swap the copy image vector with the original image vector (png).
	constexpr uint8_t 
		...
		uint64_t chunk_length = 0;
			...
			if (png[new_chunk_index + 4] == 0x78 && png[new_chunk_index + 5] == 0x5E && png[new_chunk_index + 6] == 0x5C) break;
	...
	uint8_t color_type = png[COLOR_TYPE_INDEX];
...
static const uint64_t encryptDataFile(std::vector<uint8_t>& profile, std::vector<uint8_t>& data, std::string& data_filename, bool hasMastodonOption) {
    ...
    std::transform(data_filename.begin(),
                   data_filename.begin() + data_filename_length,
                   profile.begin() + DATA_FILENAME_XOR_KEY_INDEX,
                   profile.begin() + DATA_FILENAME_INDEX,
	...
	try {
		...
		if (args.mode == Mode::conceal) {  
			...
			bool 
				...
				hasLodeModifiedImage    = false;
				
			imageColorCheck(png_vec, hasLodeModifiedImage, hasTwitterBadDims);
			...
			// If LodePNG has re-encoded the cover image, it will have also removed non-required chunks and any trailing data after IEND, so skip function.
			// If image not modified, call function to remove trailing data and superfluous chunks.
			if(!hasLodeModifiedImage) {
				removeSuperfluousChunks(png_vec);
			}
			...
			if (hasRedditOption) {
				...
				png_vec.insert(png_vec.end() - PNG_IEND_BYTES, reddit_vec.begin(), reddit_vec.end());

...
namespace {
    ...
    [[nodiscard]] std::optional<std::size_t> searchTwoByteSig(std::span<const Byte> v, Byte first, Byte second) {
        if (v.size() < 2) {
            return std::nullopt;
        }
        ...
        while (remaining > 0) {
            ...
            const auto advanced = static_cast<std::size_t>(pos - cursor) + 1;
            cursor = pos + 1;
            remaining -= advanced;
        }
    ...
    [[nodiscard]] std::optional<std::size_t> searchSig(std::span<const Byte> v, std::span<const Byte> sig, std::size_t limit = 0) {
        ...
        if (sig.size() == 1) {
            ...
            const void* hit = std::memchr(haystack.data(), sig[0], haystack.size());
            if (hit == nullptr) {
                return std::nullopt;
            }
            return static_cast<std::size_t>(static_cast<const Byte*>(hit) - haystack.data());
        }
        if (sig.size() == 2) {
            return searchTwoByteSig(haystack, sig[0], sig[1]);
        }

        auto it = std::ranges::search(haystack, sig);
        return it.empty() ? std::nullopt : std::optional(static_cast<std::size_t>(it.begin() - v.begin()));
    }
    ...
    [[nodiscard]] std::optional<JpegSegment> findJpegHeaderSegment(std::span<const Byte> jpg, Predicate predicate) {
        ...
        while (pos < jpg.size()) {
            ...
            const std::size_t segment_length =
                (static_cast<std::size_t>(jpg[pos]) << 8) |
                static_cast<std::size_t>(jpg[pos + 1]);

            if (segment_length < 2 || segment_length > jpg.size() - pos) {
                return std::nullopt;
            }
    ...
    [[nodiscard]] std::optional<uint16_t> exifOrientation(std::span<const Byte> jpg) {
        ...
        auto read16 = [&](std::size_t offset) -> uint16_t {
            ...
            if (is_le) {
                return static_cast<uint16_t>(static_cast<uint16_t>(tiff_data[offset]) |
                                             (static_cast<uint16_t>(tiff_data[offset + 1]) << 8));
            }
            return static_cast<uint16_t>((static_cast<uint16_t>(tiff_data[offset]) << 8) |
                                         static_cast<uint16_t>(tiff_data[offset + 1]));
        };
        ...
        const uint32_t ifd_offset = read32(4);

        // Need at least 2 bytes at ifd_offset for the entry count.
        if (ifd_offset < 8 ||
            static_cast<std::size_t>(ifd_offset) + 2 > tiff_data.size()) {
            return std::nullopt;
        }

        const uint16_t entry_count = read16(ifd_offset);
        std::size_t current_entry = static_cast<std::size_t>(ifd_offset) + 2;
        ...
        const std::size_t max_entries =
            (tiff_data.size() - current_entry) / ENTRY_SIZE;
        const uint16_t bounded_count =
            static_cast<uint16_t>(std::min<std::size_t>(entry_count, max_entries));

        for (uint16_t i = 0; i < bounded_count; ++i) {
            const uint16_t tag_id = read16(current_entry);
            if (tag_id == TAG_ORIENTATION) {
                const uint16_t type = read16(current_entry + 2);
                const uint32_t count = read32(current_entry + 4);
    ...
    void resizeImage(vBytes& image_file_vec, const DecodedImage& source, vBytes& resize_scratch, int quality_val, int decrease_dims_val) {
        ...
        if (tjCompress2(compressor.get(), resize_scratch.data(), newWidth, 0, newHeight, source.pixel_format, &jpegBuf.data, &jpegSize, subsamp, quality_val, flags) != 0) {
...
bool ensureImageCompatible(vBytes& image_file_vec) {
    ...
    for (int attempt = 1; attempt <= MAX_RESIZE_ATTEMPTS; ++attempt) {
        ...
        resizeImage(image_file_vec, pristine, resize_scratch, quality_val, decrease_dims_val);

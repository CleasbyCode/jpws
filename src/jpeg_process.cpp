...
namespace {
    ...
    [[nodiscard]] bool containsCommentBlockClose(std::span<const Byte> jpg) {
        return searchSig(jpg, COMMENT_BLOCK_CLOSE_SIG).has_value();
    }
    ...
    [[nodiscard]] bool isAlreadyCompatible(std::span<const Byte> jpg) {
        return jpg.size() > JFIF_COMPATIBILITY_MARKER_INDEX &&
               jpg[JFIF_COMPATIBILITY_MARKER_INDEX] == 0x19 &&
               hasCompatibleJfifHeader(jpg) &&
               !containsCommentBlockClose(jpg);
    }
    ...
    void resizeImage(
        ...
        resize_scratch.resize(checkedPixelBufferSize(new_width, new_height, source.bytes_per_pixel));

        if (!stbir_resize_uint8_srgb(source.pixels.data(), source.size.width, source.size.height, 0, resize_scratch.data(), new_width, new_height, 0, source.resize_pixel_layout)) {
    ...
    [[nodiscard]] bool recompressSameDimensionsUntilCommentBlockFree(vBytes& image_file_vec) {
        ...
        constexpr auto SAME_DIMENSION_CANDIDATES = std::to_array<EncodeCandidate>({
        ...
        const auto tryCandidates = [&](std::span<const EncodeCandidate> candidates) {
        ...
        return tryCandidates(SAME_DIMENSION_CANDIDATES);
    }
    ...
    [[nodiscard]] bool resizeUntilCommentBlockFree(vBytes& image_file_vec) {
        ...
        constexpr auto RESIZE_CANDIDATES = std::to_array<EncodeCandidate>({
            EncodeCandidate{ .subsamp = TJSAMP_444, .flags = PROGRESSIVE_JPEG_FLAGS, .label = "4:4:4 default" },
            EncodeCandidate{ .subsamp = TJSAMP_444, .flags = PROGRESSIVE_JPEG_FLAGS | TJFLAG_ACCURATEDCT, .label = "4:4:4 accurate" },
            EncodeCandidate{ .subsamp = TJSAMP_444, .flags = PROGRESSIVE_JPEG_FLAGS | TJFLAG_FASTDCT, .label = "4:4:4 fast" }
        });
        ...
        for (int attempt = 1; attempt <= MAX_RESIZE_ATTEMPTS; ++attempt) {
            ...
            const int quality_val = std::clamp(
                ...
                MIN_JPEG_QUALITY,
                MAX_JPEG_QUALITY);
            const auto& candidate = RESIZE_CANDIDATES[static_cast<std::size_t>(attempt - 1) % RESIZE_CANDIDATES.size()];
...
bool makeTailRetryCandidate(std::span<const Byte> source_jpg, vBytes& out, int retry_index) {
    ...
    const int same_dimension_candidate_count =
        QUALITY_CANDIDATE_COUNT * static_cast<int>(SAME_DIMENSION_CANDIDATES.size());

    if (retry_index < same_dimension_candidate_count) {
        const int quality_val = START_QUALITY -
            (retry_index / static_cast<int>(SAME_DIMENSION_CANDIDATES.size()));
        const auto& candidate =
            SAME_DIMENSION_CANDIDATES[static_cast<std::size_t>(retry_index) % SAME_DIMENSION_CANDIDATES.size()];

        printEncodeProgress("Tail retry", candidate.label, quality_val, source.size.width, source.size.height);
        compressPixelsToJpeg(out, source.pixels, source.size, source.pixel_format, compressor.get(), quality_val, candidate);
    ...
    const int quality_val = std::clamp(
        ...
        MIN_JPEG_QUALITY,
        MAX_JPEG_QUALITY);
    const auto& candidate = RESIZE_CANDIDATES[static_cast<std::size_t>(retry_index) % RESIZE_CANDIDATES.size()];

...
namespace {
    ...
    [[nodiscard]] std::optional<std::size_t> searchSig(std::span<const Byte> v, std::span<const Byte> sig, std::size_t limit = 0) {
        auto it = std::ranges::search((limit == 0 || limit > v.size()) ? v : v.first(limit), sig);
    ...
    [[nodiscard]] ImageSize readJpegSize(tjhandle handle, std::span<const Byte> jpg, std::string_view error_prefix) {
        ...
        if (tjDecompressHeader3(handle, jpg.data(), static_cast<unsigned long>(jpg.size()), &size.width, &size.height, &jpeg_subsamp, &jpeg_colorspace) != 0) {
    ...
    void resizeImage(vBytes& image_file_vec, int quality_val, int decrease_dims_val) {
        auto decompressor = makeHandle(tjInitDecompress(), "tjInitDecompress()");
        const auto image_size = readJpegSize(decompressor.get(), image_file_vec, "tjDecompressHeader3");

        if (image_size.width < decrease_dims_val || image_size.height < decrease_dims_val) {
        ...
        if (tjDecompress2(decompressor.get(), image_file_vec.data(), static_cast<unsigned long>(image_file_vec.size()), decoded_image_vec.data(), image_size.width, 0, image_size.height, PIXEL_FORMAT, 0) != 0) {
            throw std::runtime_error(std::format("tjDecompress2: {}", tjGetErrorStr2(decompressor.get())));
        }

        int
            newWidth  = image_size.width  - decrease_dims_val,
            newHeight = image_size.height - decrease_dims_val;
        ...
        if (!stbir_resize_uint8_srgb(decoded_image_vec.data(), image_size.width, image_size.height, 0, resized_image_vec.data(), newWidth, newHeight, 0, STBIR_4CHANNEL)) {
...
bool ensureImageCompatible(vBytes& image_file_vec) {
    ...
    const vBytes pristine = image_file_vec;

    for (;;) {
        image_file_vec = pristine;

...
namespace {
    ...
    void unlinkOutputFileBestEffort(const std::string& output_filename) noexcept {
        while (::unlink(output_filename.c_str()) != 0 && errno == EINTR) {
        }
    }
...
int main(int argc, char** argv) {
    try {
        ...
        if (pwsh_file_vec.size() < MIN_SCRIPT_PAYLOAD_SIZE) {
        ...
        profile_vec.reserve(checkedAdd(profile_vec.size(), pwsh_file_vec.size(), "ICC profile"));
        ...
        const std::size_t
            SEGMENT_SIZE = checkedSub(checkedAdd(profile_vec.size(), JFIF_SIG_LENGTH, "Profile segment"), SEGMENT_SIZE_FIELD_INDEX, "Profile segment"),
            PROFILE_SIZE = checkedSub(SEGMENT_SIZE, 16, "ICC profile");

        constexpr std::size_t MAX_PROFILE_SEGMENT_SIZE = 10 * 1024;

        if (SEGMENT_SIZE > MAX_PROFILE_SEGMENT_SIZE) {
        ...
        // Write size fields directly into profile_vec; the polyglot is streamed
        // to disk as three parts, avoiding a multi-MB insert + reserve realloc.
        constexpr std::size_t
            PROFILE_SEGMENT_SIZE_FIELD = SEGMENT_SIZE_FIELD_INDEX - JFIF_SIG_LENGTH,
            PROFILE_PROFILE_SIZE_FIELD = PROFILE_SIZE_FIELD_INDEX - JFIF_SIG_LENGTH;
        writeBigEndian(profile_vec, PROFILE_SEGMENT_SIZE_FIELD, SEGMENT_SIZE, 2);
        writeBigEndian(profile_vec, PROFILE_PROFILE_SIZE_FIELD, PROFILE_SIZE, 4);
        ...
        const std::size_t total_output_size = checkedAdd(image_file_vec.size(), profile_vec.size(), "Output image");
        const std::string OUTPUT_FILENAME = writeOutputFile({
            std::span<const Byte>(image_file_vec.data(), JFIF_SIG_LENGTH),
            std::span<const Byte>(profile_vec.data(), profile_vec.size()),
            std::span<const Byte>(image_file_vec.data() + JFIF_SIG_LENGTH, image_file_vec.size() - JFIF_SIG_LENGTH)
        });

...
int main(int argc, char** argv) {
    try {
        ...
        if (std::ranges::equal(pwsh_file_vec | std::views::take(BOM_SIG.size()), BOM_SIG)) {
            pwsh_file_vec.erase(pwsh_file_vec.begin(), pwsh_file_vec.begin() + BOM_SIG.size());
        }
        ...
        image_file_vec.reserve(checkedAdd(image_file_vec.size(), profile_vec.size(), "Output image"));

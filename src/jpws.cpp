...
namespace {
    ...
    void patchImageTail(vBytes& image_file_vec, Option option) {
        constexpr auto DEFAULT_BYTES = std::to_array<Byte>({
            0x00, 0x00, 0x20, 0x20, 0x00, 0x00, 0x23, 0x3E, 0x0D, 0x23, 0x9e
        });
        constexpr auto ALT_BYTES = std::to_array<Byte>({
            0x9e, 0x23, 0x3e, 0x0d, 0x23, 0x00, 0x00, 0x20, 0x20, 0x00, 0x00
        });
        ...
        const std::span<const Byte> tail_bytes = option == Option::Alt ? std::span<const Byte>(ALT_BYTES) : std::span<const Byte>(DEFAULT_BYTES);
    ...
    [[nodiscard]] std::string writePolyglotOutput(std::span<const Byte> image_file_vec, std::span<const Byte> profile_vec) {
        if (image_file_vec.size() < JFIF_SIG_LENGTH) {
            throw std::runtime_error("Image File Error: Image too small to write output.");
        }

        return writeOutputFile({
            image_file_vec.first(JFIF_SIG_LENGTH),
            profile_vec,
            image_file_vec.subspan(JFIF_SIG_LENGTH)
        });
    }
}
...
int main(int argc, char** argv) {
    try {
        ...
        patchImageTail(image_file_vec, args.option);
        writeJfifCommentBlock(image_file_vec);

        const std::size_t total_output_size = checkedAdd(image_file_vec.size(), profile_vec.size(), "Output image");
        const std::string output_filename = writePolyglotOutput(image_file_vec, profile_vec);

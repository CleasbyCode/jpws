...
namespace {
    ...
    void syncOutputFile(const std::string& output_filename, int fd) {
        while (::fsync(fd) != 0) {
            const int error_code = errno;
            if (error_code == EINTR) {
                continue;
            }
            unlinkAndThrow(output_filename, "Write Error: Failed to sync output file", error_code);
        }
    }
    ...
    static_assert(ICC_PROFILE_TEMPLATE.size() == ICC_PROFILE_TEMPLATE_SIZE);
    ...
    void patchImageTail(vBytes& image_file_vec, Option option) {
        ...
        constexpr auto JPEG_EOI = std::to_array<Byte>({ 0xFF, 0xD9 });

        constexpr std::size_t TAIL_OFFSET = JPEG_EOI.size();
        ...
        if (!std::ranges::equal(
                std::span<const Byte>(image_file_vec).last(JPEG_EOI.size()),
                std::span<const Byte>(JPEG_EOI))) {
            throw std::runtime_error("Image File Error: Image does not end with a JPEG EOI marker.");
        }
    ...
    [[nodiscard]] std::string writeOutputFile(std::span<const Byte> bytes) {
        ...
        for (int attempt = 0; attempt < MAX_OUTPUT_ATTEMPTS; ++attempt) {
            ...
            syncOutputFile(output_filename, output_fd.get());

            const int raw_fd = output_fd.release();
            if (::close(raw_fd) != 0) {
                const int error_code = errno;
                unlinkAndThrow(output_filename, "Write Error: Failed to close output file", error_code);
            }
    ...
    [[nodiscard]] vBytes buildTailStableOutput(
        ...
        for (int attempt = 0; attempt <= MAX_TAIL_STABILITY_RETRIES; ++attempt) {
            ...
            if (attempt == 0) {
                ...
                const TailRetryStatus status = makeTailRetryCandidate(
                    compatible_image_file_vec, candidate_image_file_vec, attempt - 1);
                if (status == TailRetryStatus::exhausted) {
                    break;
                }
                if (status == TailRetryStatus::skip) {
                    continue;
                }
            }
...
int main(int argc, char** argv) {
    try {
        ...
        const std::string output_filename = writeOutputFile(output_file_vec);

...
https://github.com/CleasbyCode/jpws
    ...
    [[nodiscard]] std::runtime_error usageError(std::string_view program_name) {
        ...
        const std::string prog = fs::path(program_name).filename().string();
...
std::optional<ProgramArgs> ProgramArgs::parse(int argc, char** argv) {
    const char* program_name = argc > 0 ? argv[0] : "jpws";
    ...
    if (argc == 4) {
        if (std::string_view(argv[1]) != "-alt") throw usageError(program_name);
        out.option = Option::Alt;
        path_arg = 2;
    } else if (argc != 3) {
        throw usageError(program_name);
    }
    out.image_file_path = fs::path(argv[path_arg]);
    out.pwsh_file_path = fs::path(argv[path_arg + 1]);
